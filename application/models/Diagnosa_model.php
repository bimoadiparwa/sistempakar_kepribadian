...
class Diagnosa_model extends CI_model
{
  ...
  {
    $query = "SELECT `tmp_final`.`id_kepribadian`, MAX(`hasil_probabilitas`) as `hasil_probabilitas`,`tbl_kepribadian`.`nama_kepribadian`, `tbl_kepribadian`.`gambar`, `tbl_kepribadian`.`deskripsi` FROM `tmp_final` JOIN `tbl_kepribadian` ON `tmp_final`.`id_kepribadian` = `tbl_kepribadian`.`id_kepribadian` GROUP BY `id_kepribadian` ORDER BY `hasil_probabilitas` DESC LIMIT 1";
    return $this->db->query($query)->result_array();
  }
